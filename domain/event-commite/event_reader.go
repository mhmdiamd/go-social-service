...
type EventReaderEventCommittee interface {
	ReadCreateEventCommittee(ctx context.Context, topic event.EventKafkaEventPublisher, handler func(ctx context.Context, payload CreateEventCommitteeRequestPayload) error) error
...
func (kc *KafkaEventReaderEventCommittee) Init(ctx context.Context) error {
	...
	if err := reader.ReadCreateEventCommittee(context.Background(), event.CREATE_EVENT_COMMITTEE, kc.svc.AddEventCommittee); err != nil {
		return err
	}
...
func (kc *KafkaEventReaderEventCommittee) ReadCreateEventCommittee(ctx context.Context, topic event.EventKafkaEventPublisher, handler func(ctx context.Context, payload CreateEventCommitteeRequestPayload) error) error {
