...
type CommunityMemberRepository interface {
	...
	DeleteMemberByCommunityId(ctx context.Context, communityId int) (err error)
...
func (s service) DeleteCommunityMemberByCommunityId(ctx context.Context, communityId int) (err error) {
	// Delete member by community id
	if err = s.repo.DeleteMemberByCommunityId(ctx, communityId); err != nil {
		log.Log.Errorf(ctx, "[DeleteCommunityMemberByCommunityId, DeleteMemberByCommunityId] with error detail %v", err.Error())
		return
	}
